class StudentManager {
    constructor() {
        ...
        this.exportJsonBtn = document.getElementById('export-btn');
        this.importJsonBtn = document.getElementById('import-data-btn');
        this.jsonFileInput = document.getElementById('data-file-input');
    ...
    init() {
        ...
        if (this.classes.length === 0) {
            console.log('No classes found on first run — starting with no classes. Use 「新增班級」建立新班級。');
    ...
    deleteStudent(studentId) {
        ...
        this.saveData();
        this.renderStudents();
        this.showToast(`學生「${student.name}」已刪除。`);
    }
    ...
    addStudentsFromText() {
        ...
        if (lines.length === 0) {
            this.showToast('請在文字框中輸入至少一位學生姓名。', 2000);
            return;
        }
        const currentClass = this.getCurrentClass();
        if (!currentClass) {
        ...
        this.showToast(`已加入 ${newStudents.length} 位學生至「${currentClass.name}」。`);
    }

    // Fix for JSON import function
    importAllData(event) {
        const file = event.target.files[0];
        if (!file) {
            this.showToast('未選擇任何檔案。', 2000);
            return;
        }
        ...
        reader.onload = (e) => {
            try {
                ...
                if (importedData && Array.isArray(importedData.classes)) {
                    ...
                    // Ensure all expected properties are updated, with fallbacks to current values if not present in imported data
                    ...
                    this.selectedHomeworkItem = importedData.selectedHomeworkItem || null; // Allow null

                    // Perform any necessary data migration or default value setting for newly added properties
                    this.classes.forEach(cls => {
                        if (!cls.contactBookEntries) cls.contactBookEntries = {};
                        if (!cls.homeworkCheckEntries) {
                            cls.homeworkCheckEntries = {};
                        ...
                        // Add more migration logic here if needed for future updates
                    });

                    this.saveData();
                    this.renderClassSelector();
                    this.renderStudents();
                    this.initializeStudentsPerRowSelect(); // Ensure the select reflects imported value
                    this.showToast('所有資料已成功匯入！');
                } else {
                    this.showToast('匯入的資料格式不正確，缺少班級資料。', 3000);
                }
            } catch (error) {
                console.error('Error parsing imported data:', error);
                this.showToast('匯入檔案解析失敗，請確認檔案格式是否為有效的JSON。', 3000);
            }
        };
        reader.onerror = () => {
            this.showToast('檔案讀取失敗。', 3000);
        };
        reader.readAsText(file);
        event.target.value = null; // Clear input for next import
    }

    // Fix for JSON export function
    exportAllData() {
        const data = {
            classes: this.classes,
            currentClassId: this.currentClassId,
    ...
    async exportAllDataToExcel() {
        ...
        const settingsData = [
            ...
            { '類型': '每列學生數', '選項': this.studentsPerRow.toString() },
            { '類型': '最後選擇作業項目', '選項': this.selectedHomeworkItem || '' }
        ];
        const wsSettings = XLSX.utils.json_to_sheet(settingsData);
        XLSX.utils.book_append_sheet(wb, wsSettings, '系統設定');

        XLSX.writeFile(wb, '學生表現管理系統_所有資料.xlsx'); // Use global XLSX object
            ...
            try {
                ...
                if (settingsSheetName) {
                    ...
                    json.forEach(row => {
                        ...
                        } else if (row['類型'] === '每列學生數' && row['選項']) {
                            const parsedNum = parseInt(row['選項'], 10);
                            if (!isNaN(parsedNum) && parsedNum >= 2 && parsedNum <= 6) {
                                importedStudentsPerRow = parsedNum;
                            }
                        } else if (row['類型'] === '最後選擇作業項目' && row['選項']) {
                            importedSelectedHomeworkItem = row['選項'];
                        }
                    });
                }
    ...
    populateHomeworkToCheckSelect() {
        ...
        this.homeworkToCheckSelect.innerHTML = '<option value="">請選擇或查看作業項目</option>';
        ...
        if (homeworkFromYesterdayFull.trim() !== '') {
            ...
            homeworkItems.forEach(item => {
                if (!addedHomeworkItems.has(item)) {
                    this.homeworkToCheckSelect.innerHTML += `<option value="${item}" data-source="contact-book">前一天聯絡簿：${item}</option>`;
    ...
    bindEvents() {
        ...
        // File operations (JSON) - Fixes provided in this update
        ...
        // NEW: Event listeners for JSON import/export
        this.exportJsonBtn.addEventListener('click', () => this.exportAllData());
        this.importJsonBtn.addEventListener('click', () => this.jsonFileInput.click()); // Trigger hidden file input
        this.jsonFileInput.addEventListener('change', (e) => this.importAllData(e));

        // File operations (Excel)
        document.getElementById('export-excel-btn').addEventListener('click', () => this.exportAllDataToExcel()); // Ensure Excel export button has a listener
        document.getElementById('import-excel-btn').addEventListener('click', () => this.excelFileInput.click()); // Ensure Excel import button triggers its input
